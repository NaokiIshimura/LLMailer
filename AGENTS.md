...
│  ├─ delivery/                 # 配信中のプロセスを、あとから止められるようにする置き場
...
- **配信しているあいだは `lib/delivery/registry` に中断の合図を登録する**。
...
- **エージェントの `model` は省略できる**。省略したぶんは `--model` を渡さず、
  ...
  追加フォームの初期値も未指定にしてある（モデルを選ばずに追加すれば Claude Code 側に従う）。
- **作業ディレクトリは `~`（ホーム）から書ける**。`spawn` で起動する `claude` にシェルの展開は
