# LLMailer

メーラーの操作感で、**ローカルの Claude Code** に指示を送るクライアント。

チャット UI ではなく、受信箱・スレッド・作成ウィンドウを持つメールクライアントとして LLM を扱う。
Claude API のキーは不要で、すでにログイン済みの Claude Code をそのまま使う。

![ホーム画面と新規作成ウィンドウ](docs/images/home-compose.png)

| メールの概念 | LLMailer での意味 |
| --- | --- |
| 宛先（To） | AI エージェント（モデル・権限・作業ディレクトリの組） |
| 件名（Subject） | スレッドのお題 |
| 本文（Body） | 指示 |
| 返信 | Claude Code の応答 |
| スレッド | Claude Code のセッション（`--resume` で継続） |
| アドレス帳 | エージェント定義一覧（受信箱と並ぶフォルダの 1 つ） |
| アーカイブ | 対応が済んだスレッドを片付ける（削除ではなく、一覧から外すだけ） |
| 配信失敗通知 | 実行エラー・タイムアウト |
| 送信の取り消し | 対応中の Claude Code を途中で止める（中断） |

宛先を複数選べば、同じ指示が各エージェントへ**並行配信**され、返信が並んで届く。

## 必要なもの

- Node.js 20 以上
- ログイン済みの [Claude Code](https://claude.com/claude-code)（`claude` コマンドが PATH にあること）

API キーの設定は不要。`claude` が別の場所にある場合は環境変数 `LLMAILER_CLAUDE_BIN` にパスを指定する。

`claude` をシェルの alias や関数（認証情報を注入するラッパーなど）として定義している場合も同じく指定が要る。
LLMailer は `shell: false` で起動するため alias は展開されず、素の実行ファイルが起動して認証に失敗する。
alias と同じ処理をするシェルスクリプトを `bin/` に置き、そのパスを `LLMAILER_CLAUDE_BIN` に指定する。

```sh
# bin/llmailer-claude
#!/bin/sh
exec <alias と同じコマンド> -- /path/to/claude "$@"
```

```bash
chmod +x bin/llmailer-claude
echo "LLMAILER_CLAUDE_BIN=$PWD/bin/llmailer-claude" >> .env.local
```

`bin/` は `.gitignore` 済み。ラッパーの中身は環境ごとに変わるのでコミットしない。
`LLMAILER_CLAUDE_BIN` は**絶対パス**で指定する（`claude` はエージェントの作業ディレクトリで起動するため、相対パスだと解決できない）。

## セットアップ

```bash
npm install
npm run dev
```

`http://localhost:25110` を開く。

## 使い方

1. 左サイドバーの「✉ 新規作成」を押す
2. 宛先（エージェント）を選び、件名と本文（指示）を書く
3. 「送信」を押すと**作成ウィンドウはすぐ閉じ**、そのスレッドが開いて「対応中…」が表示される
   （応答を待つ必要はなく、他のスレッドを見に行ける。一覧のタグと右下のインジケータにも「対応中」が出る）
4. 応答が届くとスレッドに追加される（対応中のあいだは自動で取得し直す）
5. スレッド右上の「↩ 返信」で会話を続ける（Claude Code のセッションが `--resume` され、文脈が引き継がれる）

左サイドバーは上から「ホーム / メールボックス / アーカイブ / 下書き」
「宛先（エージェント）ごとのメール一覧」「アドレス帳 / 設定」の 3 段。
切り替えると検索条件はクリアされる。

宛先の名前を選ぶと、その宛先とやり取りしたスレッドだけが一覧に並ぶ（未読があれば名前の横に件数が出る）。
一覧の見出しには宛先名と権限バッジが並び、「✉ 新規作成」からその宛先を指定した作成ウィンドウを開ける。

- **返信でも宛先を選び直せる**（スレッドの途中で別のエージェントへ引き継げる。
  途中から入ったエージェントには、それまでのやり取りがまとめて渡る）。
  件名はスレッドのお題なので、返信では変えられない（お題そのものは後から変えられる。下記）。
- 本文を書いている途中で **⌘（Ctrl）+ Enter** を押すとそのまま送信できる。
  **Tab** を押すと送信ボタンへフォーカスが移る（Shift+Tab は件名・宛先へ戻る）。
- 本文の左下「テンプレート」から、登録しておいた定型文を本文へ差し込める。
  **ボタンが出るのは本文が空のあいだだけ**で、書き始めると引っ込む（書いた内容の邪魔をしない）。
  テンプレートの追加・変更・削除は設定から行う。
- メッセージ本文は **Markdown として装飾表示**される（見出し・リスト・表・コードブロック・引用・リンク）。
  生の HTML は解釈しないため、応答をそのまま描画しても安全。
- 受信メッセージには、実行コスト・所要時間・ターン数・トークン数が表示される。
- 受信メッセージの「復元コマンド」を押すと、その返信を返した Claude Code のセッションを
  手元のターミナルで開き直すコマンド（`cd <作業ディレクトリ> && claude --resume <セッション ID>`）が
  クリップボードへ入る。セッションは作業ディレクトリごとに分かれているため `cd` まで含める。
  ターミナルから開いたセッションには、エージェントの役割（`--append-system-prompt`）と
  権限の指定（`--allowedTools` など）は引き継がれない。
- スレッドを開くと、その時点で届いているメッセージが既読になる。
  **開いたままにしているあいだに届いた返信は未読のまま残り**、本文の上に
  「新しい返信が N 件届きました」と出る。
  席を外していても、戻ってきたときに返信が届いたことに気づける。
  戻ってきて**本文をクリックする・スクロールするなど操作すれば**、読み始めたものとして
  知らせが消え、既読になる（「既読にする」を押して消すこともできる）。
- 送信そのものが失敗した場合（サーバーに届かなかった場合）は、入力内容を保ったまま作成ウィンドウが戻る。
- 「対応中」は**サーバー側に保存された状態**なので、ブラウザをリロードしても・別のタブで開いても消えない。
  配信中にサーバーを止めた場合は、次のアクセス時に「配信失敗」へ倒れ、そのまま再送できる。
- **対応中の指示は途中で中断できる**。本文の「対応中…」の右にある「中断」を押すと、
  動いている Claude Code を止めて、その 1 通が「中断しました」になる
  （宛先が複数あるときは 1 件ずつ止められる）。
  中断した通からは**再送**でき、送り直さずに済ませたいときは「取り消し」を押す。
  ただし**そこまでに書き換えられたファイルは元に戻らない**。
- **再送しても取り消しても、元の「失敗しました」「中断しました」の 1 通は消えない**。
  再送・取り消しのボタンの代わりに「〇〇 に再送しました」「〇〇 に取り消しました」と出るので、
  どんなエラーがあって、それをどう片付けたのかがスレッドを読めば分かる
  （対応は済んでいるため、一覧の失敗ラベルと未読からは外れる）。
  消せるのは下書きだけで、送受信のやり取りは記録として残る。
- **スレッドのお題（件名）は後から変えられる**。本文左上の件名の横にある鉛筆アイコンを押すと
  その場で書き換えられる（**Enter** で確定、**Esc** で取り消し）。
  件名はスレッドが 1 つだけ持つため、変えると**そのスレッドの表示はすべて新しい件名になる**。
  次の返信からはエージェントにも新しい件名が伝わるが、
  Claude Code のセッションに残った過去のやり取りの件名までは遡って変わらない。
- 対応が済んだスレッドは、一覧の行か本文右上の「アーカイブ」でメールボックスから外せる。
  やり取りは消えず、アーカイブフォルダから読める（「受信箱に戻す」で戻せる）。
  アーカイブしたあとに**返信が届いた場合も、こちらから返信した場合も**、
  やり取りが再開したものとしてメールボックスへ自動で戻る。

## エージェントと権限

メール 1 通でローカルのファイルが書き換わりうるため、**権限は宛先ごとに分けている**。
左サイドバーの「アドレス帳」を選ぶと、受信箱と同じ 3 ペインでエージェントの一覧と詳細を見られる。
一覧には `フル権限` / `読み取り専用` のバッジが付き、詳細ではモデル・作業ディレクトリ・
許可ツール・無効化ツール・設定ソース・役割（システムプロンプト）を確認できる。
詳細の「✉ メールを書く」から、その宛先を指定した状態で作成ウィンドウが開く。

| 名前 | モデル | 権限 |
| --- | --- | --- |
| Opus（汎用） | opus | 読み取り専用 |
| Sonnet（バランス） | sonnet | 読み取り専用 |
| Haiku（高速） | haiku | 読み取り専用 |
| 作業担当 | opus | **フル権限**（ファイル変更・コマンド実行） |
| コードレビュアー | opus | 読み取り専用 |

エージェントにメールアドレスは無い。宛先は名前で選び、内部では画面に出さない ID で参照する。
名前は画面で宛先を見分ける唯一の手がかりなので、既にある名前での追加・変更は弾かれる（409）。

定義は `data/agents/` 配下の 2 ファイルに分かれている。
既定は `data/agents/default.json`（コミット対象・読み取りのみ）、
画面から追加したぶんは `data/agents/custom.json`（`.gitignore` 済み）に入る。
追加ウィンドウのモデルは初期状態で「Claude Code のデフォルトに任せる」になっていて、
そのまま追加すればモデルを固定しないエージェントになる。
作業ディレクトリの初期値はホーム（`~`）で、「ディレクトリを選ぶ」から辿って変えられる。

```jsonc
{
  "id": "b6f0…",                          // 追加時にサーバーが採番（手で書く必要はない）
  "name": "myrepo 担当",
  "model": "opus",                       // opus / sonnet / haiku またはフル名（省略すると Claude Code のデフォルト）
  "systemPrompt": "…",                   // --append-system-prompt に渡す役割
  "description": "…",
  "workingDirectory": "../myrepo",        // Claude Code の cwd（相対は本プロジェクトから、~ はホーム）
  "permissionMode": "bypassPermissions",  // フル権限
  "timeoutMs": 600000
}
```

### 読み取り専用の作り方（重要）

`allowedTools` は「**追加で許可する**」指定で、`~/.claude/settings.json` の
`permissions.allow`（`Write` / `Edit` / `Bash(...)` など）を打ち消せない。
確実に読み取りだけに絞るには、次の 2 つを併用する。

```jsonc
{
  "permissionMode": "manual",
  "allowedTools": ["Read", "Glob", "Grep", "WebSearch", "WebFetch"],
  "disallowedTools": ["Write", "Edit", "MultiEdit", "NotebookEdit",
                      "Bash", "BashOutput", "KillShell", "KillBash", "Task"],
  "settingSources": ["project", "local"]   // 'user' を外してグローバル allow を継承しない
}
```

deny（`disallowedTools`）は allow より優先される。既定の読み取り専用エージェントはこの形で定義済み。

## 開発

API 一覧・データの置き場所・内部の構成・設計の前提・コーディング規約・確認コマンドは
[AGENTS.md](AGENTS.md) にまとめている。

## 今後の拡張

| 項目 | 接続点 |
| --- | --- |
| ストリーミング受信 | `--output-format stream-json` と `Transport.deliverStream()` を追加し SSE 化 |
| コードのシンタックスハイライト | `MarkdownBody` の `code` コンポーネントを差し替える |
| エージェントの詳細項目の編集 | 許可ツール・無効化ツール・設定ソースを編集フォームに足す（現在は `data/agents/custom.json` を直接編集） |
| 実メール連携 | `Transport` を満たす `MailTransport`（SMTP/IMAP）を追加 |
| SQLite 移行 | `lib/store` のリポジトリ実装を差し替え |
