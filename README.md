...
3. 「送信」を押すと**作成ウィンドウはすぐ閉じ**、そのスレッドが開いて「配信中…」が表示される
   （応答を待つ必要はなく、他のスレッドを見に行ける。右下にも「配信中」インジケータが出る）
4. 応答が届くとスレッドに追加される
...
- 新規送信でもスレッド ID をクライアントで決めるため、応答を待たずにそのスレッドを開ける
  （保存前の内容は送信リクエストと POST のレスポンスから補って表示し、再取得が終わると実データへ入れ替わる）
...
| GET | `/api/threads?folder=&q=` | スレッド一覧・下書き一覧・未読件数 |
| GET | `/api/threads/[id]` | スレッド詳細 |
| PATCH | `/api/threads/[id]` | スレッドを既読にする |
| POST | `/api/messages` | 送信し、宛先ごとの返信を得る |
